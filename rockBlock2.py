...
class rockBlockProtocol(object):
    ...
    def rockBlockServiceAvailability(self,status):pass
    def rockBlockCIEREventReporting(self,event):pass
    
    def rockBlockNetworkTimeFail(self):pass
    ...
    #SBD SESSION
    def sbdSessionResult(self,moStatus, moMsn, mtStatus, mtMsn, mtLength, mtQueued):pass
    
    #SERIAL
    def serialErrorReporting(self,msg):pass
...
class rockBlock(object):
    ...
    def __init__(self, portId, callback = None):
        
        self.s = None
        self.portId = portId
        self.callback = callback
        self.autoSession = False     #When True, we'll automatically initiate additional sessions if more messages to download
        self.skipSignalCheckMode = True
        self.lastSBDSessionWasGood = False
    ...
    def ping(self):
        ...
        if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
            self.callback.serialErrorReporting("Error_checking_modem_status_AT")
        return False
    ...
    def requestSignalStrength(self):
        ...
        if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
            self.callback.serialErrorReporting("Error_checking_signal_strength")
        return -1   
    ...
    def disableCIER(self):
        self._ensureConnectionStatus()
                
        command = "AT+CIER=0,0,0"
    ...
    def monitorCIER(self):
        ...
        while (True):
            ...
            if ( line.startswith('+CIEV:0,') ):
                signal = line[8:9]
                if(self.callback != None and callable(self.callback.rockBlockCIEREventReporting) ): 
                    self.callback.rockBlockCIEREventReporting(line)
                if(self.callback != None and callable(self.callback.rockBlockSignalUpdate) ): 
                    self.callback.rockBlockSignalUpdate(signal)
                    
            elif ( line.startswith('+CIEV:1,') ):
                status = line[8:9]
                if(self.callback != None and callable(self.callback.rockBlockCIEREventReporting) ): 
                    self.callback.rockBlockCIEREventReporting(line)
                if(self.callback != None and callable(self.callback.rockBlockServiceAvailability) ): 
                    self.callback.rockBlockServiceAvailability(status)
        ...
        if( self._queueMessage(msg) and self._attemptConnection()  ):
            ...
            while(True):
                ...
                if(SESSION_ATTEMPTS == 0):
                    #TODO: clear MO buffer ???
                    self._clearMoBuffer()
                    break

                SESSION_ATTEMPTS = SESSION_ATTEMPTS - 1
                
                if( self._attemptSession(msg) ):
    ...
    def _queueMessage(self, msg):
        ...
        if( len(msg) > 1960):
            print("sendMessageWithBytes bytes should be <= 1960 bytes")
            if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
                self.callback.serialErrorReporting("Error_sending_message_greater_than_1960bytes")
            return False
        ...
        if(self.s.readline().strip() == command):
            ...
            if(self.s.readline().strip() == "READY"):
                ...
                if (result == False):
                    if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
                        self.callback.serialErrorReporting("Error_queueing_message_writing_binary_msg")
                
                return result
                    
        if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
            self.callback.serialErrorReporting("Error_queueing_message")
        return False
    ...
    def _enableEcho(self):
        ...
        if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
            self.callback.serialErrorReporting("Error_enabing_echo")
        return False
    ...
    def _disableFlowControl(self):
        ...
        if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
            self.callback.serialErrorReporting("Error_disabling_flow_control")
        return False
    ...
    def _disableRingAlerts(self):
        ...
        if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
            self.callback.serialErrorReporting("Error_disabling_ring_alert")
        return False
                 
                 
    def _attemptSession(self, txmsg=""):
        self._ensureConnectionStatus()
        ...
        while(True):
            ...
            commandResponse = self.s.readline().strip()
            if( commandResponse == command ):
                ...
                if( response.find("+SBDIX:") >= 0 ):
                    ...
                    if(self.callback != None and callable(self.callback.sbdSessionResult) ):   
                        self.callback.sbdSessionResult(moStatus, moMsn, mtStatus, mtMsn, mtLength, mtQueued)
        
                    #Mobile Originated
                    if(moStatus <= 4):
                        self.lastSBDSessionWasGood = True
                    ...
                    if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
                        self.callback.serialErrorReporting("Error_reading_serial_after_SBDIX:" + response)
            else:
                if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
                    self.callback.serialErrorReporting("Error_reading_serial_after_AT+SBDIX:" + commandResponse)
    ...
    def _attemptConnection(self):
        ...
        if (self. skipSignalCheckMode == True) and (self.lastSBDSessionWasGood == True):
            return True
        ...
        while True:
            ...
            if(TIME_ATTEMPTS == 0):
                ...
                if(self.callback != None and callable(self.callback.rockBlockNetworkTimeFail) ): 
                    self.callback.rockBlockNetworkTimeFail()
    ...
    def _isNetworkTimeValid(self):
        ...
        if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
            self.callback.serialErrorReporting("Error_reading_network_time")
        return False
    ...
    def _clearMoBuffer(self):
        ...
        if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
            self.callback.serialErrorReporting("Error_cleaning_MO_buffer")
        return False
    ...
    def _ensureConnectionStatus(self):
        ...
        if(self.s == None or self.s.isOpen() == False):
            if(self.callback != None and callable(self.callback.serialErrorReporting) ):   
                self.callback.serialErrorReporting("Error_checking_serial_port_connection_status")
            raise rockBlockException()